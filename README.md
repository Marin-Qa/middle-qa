...
2. Открой джоб **allure-report**, в правой колонке **Job artifacts** нажми **Download**.
3. Распакуй скачанный архив.
4. Зайди в папку с отчётом и подними локальный HTTP-сервер (Allure использует относительные пути, поэтому просто открыть `index.html` в браузере может не сработать):
...
5. В браузере открой **http://localhost:8085** — отчёт отобразится корректно.
