...
| Класс | Что проверяет |
|-------|---------------|
| `CreateUserTest` | Создание пользователя через `/api/users/create` с проверкой ответа и получения созданного пользователя через `/api/users/{id}`. |
| `DeleteUserTest` | Удаление пользователя через `/api/users/{id}` и проверка, что пользователь удален (`404`). |
...
| `GetUsersTest` | Получение всех пользователей из базы `/api/users`. |
| `PutUserTest` | Обновление пользователя через `/api/users/{id}` и проверка корректного изменения данных. |
...
| Класс | Что проверяет |
|-------|---------------|
| `UserManagementTestContainersTest` | Полный набор CRUD-тестов: создание, получение, обновление, удаление, фильтрация пользователей с пагинацией и фильтрами. |
...
| Класс | Что проверяет |
|-------|---------------|
| `SyncUsersWireMockTest` | Проверяет `/api/users/sync` с разным лимитом пользователей и валидирует ответ согласно мок-файлам. |
...
| Класс | Что проверяет                                                                                                                                     |
|-------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| `UserE2ETest` | Полный пользовательский workflow(ипровизированный): синхронизация, получение, обновление, создание и удаление пользователя, проверка всех этапов. |
